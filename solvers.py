...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        return [(param, param - self.learning_rate * grad_tensors[i]) for i, param in enumerate(param_vars)]
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
        vel_updates = [(vel, self.momentum * vel - self.learning_rate * grad_tensors[i]) for i, vel in enumerate(vel_vars)]
        param_updates = [(param, param + vel_updated[1]) for param, vel_updated in zip(param_vars, vel_updates)]
        return vel_updates + param_updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            session.run(update_ops, feed_dict=dict(zip(placeholders, train_data)))
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        # You may want to initialize some variables that are shared across iterations
        "*** YOUR CODE HERE ***"
        train_iterator = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        val_iterator = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        loss_tensor = self.get_loss_tensor(model.prediction_tensor, target_ph, model.get_param_vars(regularizable=True))
        updates = self.get_updates(loss_tensor, model.get_param_vars(trainable=True))
        update_ops = [tf.assign(old_var, new_var_or_tensor) for (old_var, new_var_or_tensor) in updates]
        train_losses = []
        ...
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            train_item = train_iterator.next()
            train_feed_dict = dict(zip(placeholders, train_item))
            val_item = val_iterator.next()
            val_feed_dict = dict(zip(placeholders, val_item))
            train_loss = session.run(loss_tensor, feed_dict=train_feed_dict)
            session.run(update_ops, feed_dict=train_feed_dict)
            val_loss = session.run(loss_tensor, feed_dict=val_feed_dict)
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        # You may want to initialize some variables that are shared across iterations
        "*** YOUR CODE HERE ***"
        train_iterator = MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        val_iterator = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        loss_tensor = self.get_loss_tensor(model.prediction_tensor, target_ph, model.get_param_vars(regularizable=True))
        updates = self.get_updates(loss_tensor, model.get_param_vars(trainable=True))
        update_ops = [tf.assign(old_var, new_var_or_tensor) for (old_var, new_var_or_tensor) in updates]
        train_losses = []
